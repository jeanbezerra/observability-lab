...
if [[ -n "${K8S_SECRETS_FILE:-}" ]]; then
  if [[ ! -r "${K8S_SECRETS_FILE}" ]]; then
    printf 'ERRO: arquivo de segredos OIDC não pode ser lido: %s\n' "${K8S_SECRETS_FILE}" >&2
    exit 1
  fi
  # shellcheck source=/dev/null
  source "${K8S_SECRETS_FILE}"
elif [[ -r "${PROJECT_DIR}/oidc-secrets.env" ]]; then
  K8S_SECRETS_FILE="${PROJECT_DIR}/oidc-secrets.env"
  # shellcheck source=/dev/null
  source "${K8S_SECRETS_FILE}"
fi
...
DASHBOARD_DEFAULT_LANGUAGE="${DASHBOARD_DEFAULT_LANGUAGE:-en}"
...
OIDC_USERNAME_PREFIX="${OIDC_USERNAME_PREFIX:-oidc:}"
OIDC_GROUPS_PREFIX="${OIDC_GROUPS_PREFIX:-oidc:}"
OIDC_VIEWER_GROUP="${OIDC_VIEWER_GROUP:-k8s-viewers}"
OIDC_ADMIN_GROUP="${OIDC_ADMIN_GROUP:-k8s-admins}"
OIDC_ENABLE_ADMIN_GROUP="${OIDC_ENABLE_ADMIN_GROUP:-true}"
OIDC_CA_FILE="${OIDC_CA_FILE:-}"
HEADLAMP_EXTERNAL_URL="${HEADLAMP_EXTERNAL_URL:-}"
HEADLAMP_OIDC_SCOPES="${HEADLAMP_OIDC_SCOPES:-openid,profile,email}"
...
desired_state_fingerprint() {
  printf '%s\n' \
    ...
    "OIDC_VIEWER_GROUP=${OIDC_VIEWER_GROUP}" \
    "OIDC_ADMIN_GROUP=${OIDC_ADMIN_GROUP}" \
    "OIDC_ENABLE_ADMIN_GROUP=${OIDC_ENABLE_ADMIN_GROUP}" \
    "HEADLAMP_EXTERNAL_URL=${HEADLAMP_EXTERNAL_URL}" \
    "HEADLAMP_OIDC_CLIENT_SECRET_SHA256=$(printf '%s' "${HEADLAMP_OIDC_CLIENT_SECRET}" | sha256sum | awk '{print $1}')" \
