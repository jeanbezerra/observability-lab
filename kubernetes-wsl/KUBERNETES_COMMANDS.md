...
Execute dentro do Ubuntu 26.04 no WSL 2:
...
### Encerramento do WSL e `cri-containerd-*.scope`
...
sudo journalctl -b -u k8s-wsl-node-ip -u kubelet -u containerd --no-pager
sudo crictl ps -a
```
...
# O Service deve permanecer interno, sem NodePort
kubectl get service headlamp -n "$DASHBOARD_NS" \
  -o jsonpath='{.spec.type}{" nodePort="}{.spec.ports[0].nodePort}{"\n"}'

# Reinício normal
kubectl rollout restart deployment/headlamp -n "$DASHBOARD_NS"

# Túnel persistente e restrito a localhost
...
sudo env K8S_CONFIG_FILE="$(realpath cluster.env)" \
  bash scripts/70-configure-local-access.sh
```
...
Prefira executar os scripts: eles validam que o recurso pertence ao instalador, aguardam o rollout e restauram o acesso local sem expor a porta na rede.
